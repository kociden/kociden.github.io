...
const SignalSite: React.FC<SiteProps> = ({ selectedNewsId, onSelectNews }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0b] text-zinc-100 font-sans antialiased selection:bg-emerald-400 selection:text-black">
      {/* Spotlight */}
      <div
        ref={spot}
        className="pointer-events-none fixed inset-0 z-0"
        style={{ background: 'radial-gradient(600px circle at var(--x, 50%) var(--y, 30%), rgba(16,185,129,.08), transparent 60%)' }}
      />
      ...
      <header className="fixed top-0 inset-x-0 z-40 border-b border-white/5 bg-[#0a0a0b]/70 backdrop-blur-md">
        <div className="mx-auto max-w-6xl px-6 h-14 flex items-center justify-between">
          ...
          <a href={`mailto:${PROFILE.social.email}`} className="text-xs font-medium rounded-full bg-white text-black px-4 py-1.5 hover:bg-emerald-400 transition-colors">
            Contact
          </a>
        </div>
      </header>
        ...
        {news ? (
          ...
          <article className="mx-auto max-w-3xl px-6 pt-32 pb-24">
            ...
            <div className="flex items-center gap-3 font-mono text-xs text-zinc-500 mb-4">
              <time>{news.date}</time>
              {news.tag && <span className="rounded border border-emerald-400/30 text-emerald-400 px-1.5 py-0.5 text-[10px] tracking-widest uppercase">{news.tag}</span>}
            </div>
            ...
            <div className="news-body text-zinc-300 text-[17px] [&_a]:text-emerald-400 [&_h3]:text-white" dangerouslySetInnerHTML={{ __html: news.content ?? '' }} />
            {news.url && (
              <a href={news.url} target="_blank" rel="noopener noreferrer" className="mt-10 inline-flex items-center gap-2 text-sm text-emerald-400 hover:underline underline-offset-4">
          ...
          <>
            ...
            <section className="relative overflow-hidden">
              ...
              <div className="absolute -top-40 left-1/2 -translate-x-1/2 h-[520px] w-[820px] rounded-full bg-emerald-500/10 blur-[120px] signal-glow" />
              <div className="relative mx-auto max-w-6xl px-6 pt-44 pb-32">
                ...
                <h1 className="reveal reveal-delay-1 font-grotesk text-[13vw] md:text-[7.5rem] leading-[0.92] font-bold tracking-[-0.04em]">
                  Build worlds.<br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 via-white to-zinc-500">Ship fast.</span>
                </h1>
                <div className="reveal reveal-delay-2 mt-10 grid md:grid-cols-[1fr_auto] gap-8 items-end">
                  ...
                  <div className="flex gap-3">
                    <a href="#works" className="inline-flex items-center gap-2 rounded-md bg-white text-black px-5 py-3 text-sm font-semibold hover:bg-emerald-400 transition-colors">
                ...
                <div className="mt-10 grid sm:grid-cols-2 gap-px bg-white/5 border border-white/5 rounded-lg overflow-hidden">
                  {SKILLS.map((s) => (
                    <div key={s.group} className="bg-[#0a0a0b] p-5">
                      <div className="font-mono text-[11px] tracking-[0.2em] uppercase text-emerald-400 mb-3">{s.group}</div>
                    ...
                    <li key={w.id} className="reveal group">
                      <a href={w.url ?? '#works'} target={w.url ? '_blank' : undefined} rel="noopener noreferrer" className="grid md:grid-cols-12 gap-6 py-8 items-center">
                        ...
                        <div className="md:col-span-5">
                          <h3 className="font-grotesk text-2xl font-bold tracking-tight group-hover:text-emerald-400 transition-colors flex items-center gap-2">
                            {w.title}
                ...
                <ul className="grid md:grid-cols-2 gap-4">
                  {NEWS_DATA.map((n) => (
                    <li key={n.id} className="reveal">
                      <button onClick={() => onSelectNews(n.id)} className="group w-full text-left rounded-lg border border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-emerald-400/40 transition-colors p-6 flex gap-5">
                        {n.thumbnail && (
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-3 font-mono text-[11px] text-zinc-500 mb-2">
                            <time>{n.date}</time>
                            {n.tag && <span className="text-emerald-400 tracking-widest uppercase">{n.tag}</span>}
                          </div>
                          <h3 className="font-semibold leading-snug group-hover:text-emerald-400 transition-colors line-clamp-2">{n.title}</h3>
            ...
            <section id="contact" className="border-t border-white/5 relative overflow-hidden">
              ...
              <div className="relative mx-auto max-w-6xl px-6 py-32 text-center">
                ...
                <h2 className="reveal font-grotesk text-5xl md:text-7xl font-bold tracking-[-0.04em] mt-8">
                  一緒に、<span className="text-emerald-400">ワクワク</span>を作りませんか。
                </h2>
                ...
                <div className="reveal reveal-delay-2 mt-10 flex flex-wrap justify-center gap-3">
                  <a href={`mailto:${PROFILE.social.email}`} className="inline-flex items-center gap-2 rounded-md bg-white text-black px-6 py-3.5 font-semibold hover:bg-emerald-400 transition-colors">
